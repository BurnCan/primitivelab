...
for(std::size_t sourceIndex:displayOrder){auto& o=*objects[sourceIndex];ImGui::PushID(static_cast<int>(sourceIndex));
 ...
 if(objectExpanded){char name[128];std::snprintf(name,sizeof(name),"%s",o.name.c_str());if(ImGui::InputText("Name",name,sizeof(name)))o.name=name;
  if(renderable){int mode=int(o.renderMode);const char* modes[]={"Texture","Material"};ImGui::TextUnformatted("Render Mode");ImGui::SetNextItemWidth(-1.0f);if(ImGui::Combo("##RenderMode",&mode,modes,2))o.renderMode=RenderMode(mode);}
  if(renderable&&o.renderMode==RenderMode::Material){const auto& ids=materials.GetIds();ImGui::TextUnformatted("Material");if(ids.empty())ImGui::TextDisabled("No materials available");else{std::string preview=o.materialId.empty()?"(missing)":o.materialId;ImGui::SetNextItemWidth(-1.0f);if(ImGui::BeginCombo("##Material",preview.c_str())){for(const auto&id:ids){bool selected=id==o.materialId;if(ImGui::Selectable(id.c_str(),selected))o.materialId=id;if(selected)ImGui::SetItemDefaultFocus();}ImGui::EndCombo();}}auto warning=materials.GetWarning(o.materialId);if(!warning.empty())ImGui::TextWrapped("Material warning: %s",warning.c_str());}
  if(renderable&&o.renderMode==RenderMode::Texture)std::visit([&](auto& payload){using T=std::decay_t<decltype(payload)>;if constexpr(std::is_same_v<T,Primitive2D>||std::is_same_v<T,Primitive3D>||std::is_same_v<T,Terrain>)TextureSelector(textureFiles,payload.GetTexturePath(),[&](const std::string& texture){payload.SetTexturePath(texture);});},o.payload);
